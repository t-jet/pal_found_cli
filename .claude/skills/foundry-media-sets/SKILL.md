...
Run `foundry-media-sets --help` for syntax. The CLI exposes exactly 19 Media Sets v2 operations: `media-set abort|calculate|clear|commit|create|get|get-result|get-rid-by-path|get-status|info|metadata|read|read-original|reference|register|retrieve|transform|upload|upload-media`.
