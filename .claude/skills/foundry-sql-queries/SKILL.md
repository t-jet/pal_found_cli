---
name: foundry-sql-queries
description: Run Foundry SQL Queries API v2 operations: cancel, execute, execute-ontology, get-results, and get-status.
---

# Foundry SQL Queries

Run `foundry-sql-queries --help` for syntax. The CLI exposes exactly 5 SqlQueries v2 operations on the `query` resource: `query cancel`, `query execute`, `query execute-ontology`, `query get-results`, `query get-status`.
