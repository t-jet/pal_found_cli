...
Run `foundry-streams --help` for syntax. The CLI exposes exactly 15 Streams v2 operations: `dataset create`; `stream create|get|get-end-offsets|get-records|publish-binary-record|publish-record|publish-records|reset`; `subscriber create|commit-offsets|delete|get-read-position|read-records|reset-offsets`.
