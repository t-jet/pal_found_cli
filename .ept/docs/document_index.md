...
**Major Change:** Added DESIGN-017-connectivity-cli.md and DESIGN-018-media-sets-cli.md — completed implementation designs for DEV-STORY-017 (20-operation Connectivity CLI, corrected from the stale 15-operation count) and DEV-STORY-018 (19-operation Media Sets CLI with BinaryDownloadHandler downloads and FR-ATTR-4 attribution) (2026-08-10).
**Implementation Change:** DEV-017 and DEV-018 implemented — `foundry-connectivity` (20 ops, `src/foundry_cli/connectivity/`) and `foundry-media-sets` (19 ops, `src/foundry_cli/media_sets/`) CLIs with packaged metadata allow-lists (7/13 and 5/14), skill launchers under `.claude/skills/`, pyproject entry points and package-data, and AccessControlGuard `register`/`calculate` write-verb additions for the Media Sets namespace (2026-08-10).
