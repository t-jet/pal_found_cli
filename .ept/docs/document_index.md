...
**Major Change:** Added DESIGN-013-models-cli.md — completed design for DEV-STORY-013 (foundry-models CLI, 23 Models v2 operations). Added DESIGN-014-orchestration-cli.md — completed design for DEV-STORY-014 (foundry-orchestration CLI, 20 Orchestration v2 operations). Added TESTCASE-013-test-cases.md — QA test case design for DEV-STORY-013 (28 cases, 23 foundry-models operations, full story AC coverage). Added TESTCASE-014-test-cases.md — QA test case design for DEV-STORY-014 (23 cases, 20 foundry-orchestration operations, full story AC coverage). Implemented DEV-013 (foundry-models CLI under `src/foundry_cli/models/` with Claude launcher `.claude/skills/foundry-models/`) and DEV-014 (foundry-orchestration CLI under `src/foundry_cli/orchestration/` with Claude launcher `.claude/skills/foundry-orchestration/`); updated `AccessControlGuard` write-verb classification (launch/promote/pause/unpause) per DESIGN-013/014; registered `foundry-models` and `foundry-orchestration` console entry points in pyproject.toml.
