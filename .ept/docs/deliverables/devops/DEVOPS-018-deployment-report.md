# DEVOPS-018 - Foundry Media Sets packaging and deployment report
...
Candidate `063f72d` is ready for release as a Python package plus matching Claude skill bundle. Clean-archive build (sdist + wheel per PEP 517), wheel and editable installation, console entry-point smoke tests, packaged metadata policy verification (5 PERMITTED / 14 BLOCKED), Python 3.11/3.12 gates, security gates, and rehearsed rollback all passed. No live credential, Foundry environment, cloud resource, registry, or shared-worktree package was changed.

## Candidate and baseline

The candidate came from a clean `git archive` of commit `063f72d`, which contains both the Media Sets and Connectivity namespace implementations (DEV-017/DEV-018 share base commit `62c269f`) plus the CODEREVIEW-017 P1 corrective fix committing the `file_import_filters` keyword dispatch. The rollback baseline is the parent commit `c548c1b`, which has no `media_sets/` package, no `foundry-media-sets` entry point, and no corresponding Claude skill directory.
...
Clean candidate and rollback archives contained no `.git` directory and no unrelated files from the dirty shared worktree. DESIGN-018 and TESTCASE-018 supplied the verification contracts; documentation outside the candidate commit was not copied into its package source.
...
The installed Media Sets policy has exactly 5 `PERMITTED` and 14 `BLOCKED` rows. The installed Connectivity policy has exactly 7 `PERMITTED` and 13 `BLOCKED` rows. Both match the candidate source byte-for-byte (installed=1,498B vs src=1,498B identical=True for media_sets; 1,702B identical=True for connectivity).
...
| Package imports (`foundry_cli.media_sets`, `foundry_cli.connectivity`) | PASS | PASS |
| Empty working directory after checks | Yes | Yes |

Package import probes confirmed 19 Media Sets operations and 20 Connectivity operations in the OP_SPECS catalogs. The smoke directory contained only the test `.env` file during ACL checks; no `.foundry-data` directory or stray artifacts were created by the CLI itself.
...
PERMITTED metadata operations (`foundry-media-sets media-set get ri.media-set.main.media-set.12345` and `foundry-connectivity connection get ri.connection.main.connection.12345`) passed the ACL gate and proceeded to network retries (ConnectionError against the dummy hostname, exit 6), proving the allowed paths are not blocked by the policy. Invalid RIDs returned exit 1 (UserInputError). All ACL checks executed from the installed site-packages policy files.
...
| Focused media_sets + connectivity + ACL suites (3.11) | 141 passed in 1.86 s |
| Namespace branch coverage (3.11) | media_sets 87%, connectivity 88% (both above 80%) |
...
2. Confirmed `foundry-media-sets.exe` and `foundry-connectivity.exe` were absent, and `foundry_cli.media_sets` / `foundry_cli.connectivity` had no import specification.
...
5. Confirmed `foundry-media-sets --help` and `foundry-connectivity --help` returned exit 0, imports of both namespaces succeeded, and `pip check` exited 0.
...
DEVOPS-018 is READY. Candidate `063f72d` is production-ready for controlled package and Claude skill bundle release. Publication and live deployment remain separate authorized actions. Deployment activates after the parent story DEV-STORY-018 passes QA (TESTEXEC-018).
