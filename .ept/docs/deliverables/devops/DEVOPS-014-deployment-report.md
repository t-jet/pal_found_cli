# DEVOPS-014 - Foundry Orchestration packaging and deployment report
...
| Task | DEVOPS-014 |
| Result | READY |
| Candidate commit | `bd13955` (feat(models,orchestration): add foundry-models and foundry-orchestration CLIs (DEV-013/DEV-014)) |
...
Candidate `bd13955` is ready for release as a Python package plus matching Claude skill bundle. Clean-archive build (sdist + wheel per PEP 517), wheel and editable installation, console entry-point smoke tests, packaged metadata policy verification, Python 3.11/3.12 gates, security gates, and rehearsed rollback all passed. No live credential, Foundry environment, cloud resource, registry, or shared-worktree package was changed.

## Candidate and baseline

The candidate came from a clean `git archive` of commit `bd13955`, which contains both the Models and Orchestration namespace implementations (DEV-013/DEV-014 share one commit). The rollback baseline is the parent commit `9f777fc`, which has no `orchestration/` package, no `foundry-orchestration` entry point, and no corresponding Claude skill directory.
...
Clean candidate and rollback archives contained no `.git` directory and no unrelated files from the dirty shared worktree. DESIGN-014 and TESTCASE-014 supplied the verification contracts; documentation outside the candidate commit was not copied into its package source.
...
Installed wheel metadata exposed the new command and retained every console script from the rollback baseline:
...
The installed Orchestration policy has exactly 12 `PERMITTED` and 8 `BLOCKED` rows. The installed Models policy has exactly 12 `PERMITTED` and 11 `BLOCKED` rows. Both match the canonical `.ept/docs/deliverables/architecture/metadata-allow-list.md`.
...
Package import probes confirmed 20 Orchestration operations and 23 Models operations in the OP_SPECS catalogs. The smoke directory contained only the test `.env` file; no `.foundry-data` directory or stray artifacts were created by the CLI itself.
...
2. Confirmed `foundry-orchestration.exe` and `foundry-models.exe` were absent, and `foundry_cli.orchestration` / `foundry_cli.models` had no import specification.
...
5. Confirmed `foundry-orchestration --help` and `foundry-models --help` returned exit 0, all retained command help checks passed, and `pip check` exited 0.
...
DEVOPS-014 is READY. Candidate `bd13955` is production-ready for controlled package and Claude skill bundle release. Publication and live deployment remain separate authorized actions. Deployment activates after the parent story DEV-STORY-014 passes QA (TESTEXEC-014).
