# DEVOPS-015 - Foundry SQL Queries packaging and deployment report
...
Candidate `0c88063` is ready for release as a Python package plus matching Claude skill bundle. Clean-archive build (sdist + wheel per PEP 517), wheel and editable installation, console entry-point smoke tests, packaged metadata policy verification (1 PERMITTED / 4 BLOCKED), Python 3.11/3.12 gates, security gates, and rehearsed rollback all passed. No live credential, Foundry environment, cloud resource, registry, or shared-worktree package was changed.

## Candidate and baseline

The candidate came from a clean `git archive` of commit `0c88063`, which contains both the SQL Queries and Streams namespace implementations (DEV-015/DEV-016 share one commit). The rollback baseline is the parent commit `bd13955`, which has no `sql_queries/` package, no `foundry-sql-queries` entry point, and no corresponding Claude skill directory.
...
Clean candidate and rollback archives contained no `.git` directory and no unrelated files from the dirty shared worktree. DESIGN-015 and TESTCASE-015 supplied the verification contracts; documentation outside the candidate commit was not copied into its package source.
...
The installed SQL Queries policy has exactly 1 `PERMITTED` and 4 `BLOCKED` rows. The installed Streams policy has exactly 3 `PERMITTED` and 12 `BLOCKED` rows. Both match the candidate source byte-for-byte (Compare-Object diff = 0 lines).
...
| Package imports (`foundry_cli.sql_queries`, `foundry_cli.streams`) | PASS | PASS |
| Empty working directory after checks | Yes | Yes |

Package import probes confirmed 5 SQL Queries operations and 15 Streams operations in the OP_SPECS catalogs. The smoke directory contained only the test `.env` file; no `.foundry-data` directory or stray artifacts were created by the CLI itself.
...
PERMITTED metadata operations (`foundry-sql-queries query get-status <rid>` and `foundry-streams subscriber get-read-position <rid> master sub1`) passed the ACL gate and proceeded to network retries (ConnectionError against the dummy hostname, exit 6), proving the allowed paths are not blocked by the policy. Invalid input returned exit 1 (UserInputError). All ACL checks executed from the installed site-packages policy files.
...
| Focused sql_queries + streams suites (3.11) | 57 passed in 1.81 s |
...
Namespace branch coverage on 3.11: `foundry_sql_queries_cli.py` 89%, `foundry_streams_cli.py` 90%, both above the 80% gate. Same numbers on Python 3.12.
...
2. Confirmed `foundry-sql-queries.exe` and `foundry-streams.exe` were absent, and `foundry_cli.sql_queries` / `foundry_cli.streams` had no import specification.
...
5. Confirmed `foundry-sql-queries --help` and `foundry-streams --help` returned exit 0, imports of both namespaces succeeded, and `pip check` exited 0.
...
DEVOPS-015 is READY. Candidate `0c88063` is production-ready for controlled package and Claude skill bundle release. Publication and live deployment remain separate authorized actions. Deployment activates after the parent story DEV-STORY-015 passes QA (TESTEXEC-015).
