...
The [metadata](environment_and_metadata.md#sec-metadata) of a container is information that describes the container. Apptainer automatically records important information such as the definition file used to build a container. Other details such as the version of Apptainer used are present as [labels](environment_and_metadata.md#sec-labels) on a container. You can also specify your own to be recorded against your container.
...
>   * _Most_ of the environment variables set on your host, which are passed into the container.
...
If you are interested in variables available when you are _building_ a container, rather than when running a container, see [build environment section](build_env.md#build-environment).
## Environment From a Base Image
When you build a container with Apptainer you might _bootstrap_ from a library or Docker image, or using Linux distribution bootstrap tools such as `debootstrap`, `yum`, `dnf` etc.
When using `debootstrap`, `yum`, `dnf` etc. you are starting from a fresh install of a Linux distribution into your container. No specific environment variables will be set. If you are using a `library` or `Docker` source then you may inherit environment variables from your base image.
...
Note
See compatibility documentation for `SINGULARITYENV_` prefixed environment variable support [here](singularity_compatibility.md#singularity-environment-variable-compatibility).
...
If you _do not want_ the host environment variables to pass into the container you can use the `-e` or `--cleanenv` option. This gives a clean environment inside the container, with a minimal set of environment variables for correct operation of most software.
...
Note
See compatibility documentation for `SINGULARITY_` prefixed environment variable support [here](singularity_compatibility.md#singularity-environment-variable-compatibility).
...
###  `--env-file` option
The `--env-file` option lets you provide one or more files that contain environment variables as `NAME=VALUE` pairs, e.g.:
...
$ apptainer run --env-file myenvs --env-file myenvs2 env.sif
Hello from a file 2

$ apptainer run --env-file myenvs2,myenvs env.sif
Hello from a file

```

Note
Note that if more than one file is used, the variables of the same name are overridden with later files taking priority.
###  `APPTAINERENV_` prefix
If you export an environment variable on your host called `APPTAINERENV_xxx` _before_ you run a container, then it will set the environment variable `xxx` inside the container:
...
This default behavior of Apptainer differs from Docker/OCI handling of environment variables / `ENV` directives. To avoid the extra evaluation of variables that Apptainer performs you can:
...
The `--compat` flag is a short-hand flag to activate `--no-eval` along with other Docker/OCI compatibility flags. See [Docker-like --compat Flag](docker_and_oci.md#compat-flag) for more details.
...
>   * If `PATH` is not defined set the Apptainer default `PATH` _or_
...
Note
A detailed description of what the `umask` is, and how it works can be found at [Wikipedia](https://en.wikipedia.org/wiki/Umask).
...
Apptainer container default labels are represented using the [rc1 Label Schema](http://label-schema.org/rc1/).
### Inherited Labels
...
```
  ...
  * **actions** : This directory contains helper scripts to allow the container to carry out the action commands. (e.g. `exec` , `run` or `shell`). In later versions of Apptainer, these files may be dynamically written at runtime, _and should not be modified_ in the container.
