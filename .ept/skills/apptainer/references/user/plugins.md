...
Plugins can influence the behaviour of Apptainer in specific ways:
  ...
  * A cli plugin can use the `ApptainerEngineConfig` callback to change the container configuration before it is passed to the runtime, e.g. adding bind mounts etc.
...
## Writing a Plugin
Developers interested in writing Apptainer plugins can get started by reading the [Go documentation](https://godoc.org/github.com/apptainer/apptainer/pkg/plugin) for the plugin package.
Example plugins can be found in the Apptainer [source code](https://github.com/apptainer/apptainer/tree/main/examples/plugins).
